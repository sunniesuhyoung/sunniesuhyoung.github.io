...
function compareElem(){
    ...
    function exchangeElem(index1, index2){
        ...
        var selection = {sel1: index1, sel2: index2};
        userAct.push(selection);
    ...
    function gameWin(){
        gameSuccess = true;
        sendResult(gameSuccess, userAct);
    }
...
var indices, attemptIndex;

function sendResult(gameSuccess, activity){
    xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
            attemptIndex = xmlhttp.responseText;
            if (attemptIndex >= 2 && gameSuccess == false){
                var delay=500;
                setTimeout(function() {
                    multiGameFail.style.display = "block";
                    multiFailText.style.display = "block";
                }, delay);
            } else if (attemptIndex < 3 && gameSuccess == false){
                var delay=500;
                setTimeout(function() {
                    popupGameFail.style.display = "block";
                    gameFailText.style.display = "block";
                }, delay);
            } else if (gameSuccess == true){
                var delay=500;
                setTimeout(function() {
    ...
    var array = JSON.stringify(activity);

    xmlhttp.open("POST", "getPerformance.html");
    xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlhttp.send("data1="+gameSuccess+"&data2="+array);
}
